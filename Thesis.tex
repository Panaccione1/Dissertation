...
%
\usepackage{graphicx}
\usepackage{url}          % fixes all \url{} errors in Thesis.bbl
\usepackage[T1]{fontenc}  % fixes \k{} accent in ch3
\usepackage{upgreek}      % fixes \upmu in ch3
\usepackage{hyperref}
\usepackage{amsmath}       % \text{}, \lesssim, \gtrsim, etc.
\usepackage{amssymb}       % \lesssim, \gtrsim
\usepackage{lineno}
\usepackage{booktabs}
\linenumbers
%
...
\author{Wyatt Panaccione}
\title{Investigation of Halide Perovskite Materials, Thermal Sensing Concepts, and Radiation-Tolerant FPGA Instrumentation for Advanced Radiation Monitoring}
\authordegrees{B.S.}  % Degrees thus far, not including this one.
\unit{Nuclear Engineering}

\advisorname{Dr. Praneeth Kandlakunta}
\member{Dr. Raymond Cao}
\member{Dr. Richard Vasques}
...
% and ``vita.tex''.
%
\dedication{To my Lord and Savior Jesus Christ for without I could do no things. Also to my beautiful wife, who has been my rock and support throughout this journey.  I couldn't have done it without you.}
\include{ack}
\include{vita}
...
\include{ch3.PerFn}    % what is your solution
\include{ch4.solarcells}       % conclusion stuff
\include{ch5}
\include{ch6}
\include{ch7}
\include{ch8}
