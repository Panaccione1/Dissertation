...
The research reported in Chapters~\ref{ch:CsFAPbI} and~\ref{ch:perovskite_detectors}
...
A complementary criterion is the octahedral factor $\mu = r_{\mathrm{B}}/r_{\mathrm{X}}$,
...
Chapter~\ref{ch:perovskite_detectors} addresses the neutron sensitivity limitation
...
($Z_{\mathrm{eff}} = 14$) and approaching that of cadmium zinc telluride
($Z_{\mathrm{eff}} \approx 49.1$)~\cite{Springer_CsPbBr3}.  This high
$Z_{\mathrm{eff}}$ makes CsPbBr$_3$ among the most promising semiconductor
...
organic-inorganic boundaries.  Chapter~\ref{ch:perovskite_detectors} investigates
...
fast-neutron detector.  Chapter~\ref{ch:perovskite_detectors} presents the synthesis,
...
\begin{equation}
  W \approx 2.67\,E_{\mathrm{g}} + 0.87~\text{eV},
  \label{eq:klein}
\end{equation}
where $E_{\mathrm{g}}$ is the bandgap energy~\cite{Klein1968}.  For
MAPbI$_3$ ($E_{\mathrm{g}} \approx 1.55$~eV), this gives $W \approx 5.0$~eV
per pair; for CsPbBr$_3$ ($E_{\mathrm{g}} \approx 2.3$~eV), $W \approx 7.0$~eV
...
of a spectral peak:
\begin{equation}
  R = \frac{\Delta E_{\mathrm{FWHM}}}{E_0},
  \label{eq:resolution}
\end{equation}
...
\begin{equation}
  \Delta E_{\mathrm{FWHM}}^{\mathrm{Fano}} = 2.35\,\sqrt{F \cdot W \cdot E_0},
  \label{eq:fano}
\end{equation}
...
\begin{equation}
  n_i = \sqrt{N_c N_v}\,\exp\!\left(-\frac{E_{\mathrm{g}}}{2kT}\right),
  \label{eq:ni}
\end{equation}
...
($E_{\mathrm{g}} = 0.67$~eV) has $n_i \approx 2.4 \times 10^{13}$~cm$^{-3}$,
...
($E_{\mathrm{g}} = 1.12$~eV) fares considerably better with
...
minimizes the dark current $I_{\mathrm{dark}} = V/R = VA/(L\rho)$ that flows
...
combination of large $E_{\mathrm{g}}$ (reducing $n_i$) and the absence of
...
\textbf{Effective atomic number} ($Z_{\mathrm{eff}}$).  For gamma and X-ray
detection, $Z_{\mathrm{eff}}$ determines the photoelectric stopping power,
...
millimeters of CsPbBr$_3$ ($Z_{\mathrm{eff}} = 65.9$) or CdZnTe
($Z_{\mathrm{eff}} = 49.1$) provide equivalent absorption~\cite{Murty1965}.

\textbf{Pair creation energy} ($W$, eV).  As noted in
Eq.~\ref{eq:klein}, $W$ is approximately proportional to $E_{\mathrm{g}}$.
...
\hline
Material & $E_{\mathrm{g}}$ (eV) & $Z_{\mathrm{eff}}$ &
...
\multicolumn{7}{l}{\footnotesize *For alpha particles; gamma spectroscopy
  performance is limited by the low $Z_{\mathrm{eff}}$.}
\end{tabular}
\end{table}
...
a near-ideal bandgap for thermal leakage suppression, high $Z_{\mathrm{eff}}$,
...
accumulated over three decades.  HgI$_2$ offers the highest $Z_{\mathrm{eff}}$
...
radiation hardness and thermal tolerance but its low $Z_{\mathrm{eff}}$
...
Its $Z_{\mathrm{eff}}$ of 65.9 is the highest of any material in common
...
requirements simultaneously: (1) $E_{\mathrm{g}}$ large enough to suppress
thermal leakage at the maximum operating temperature; (2) $E_{\mathrm{g}}$
small enough to keep $W$, and therefore the Fano noise, acceptably
low; and (3) $Z_{\mathrm{eff}}$ high enough for efficient photon stopping
...
energy as $\sigma_{\mathrm{pe}} \propto Z^{4.5}/E^{3.5}$, so high-$Z$
...
\begin{equation}
  Z_{\mathrm{eff}} = \left(\sum_i w_i Z_i^n\right)^{1/n},
  \label{eq:Zeff}
\end{equation}
...
of 0.229, 0.449, and 0.322, respectively) yields $Z_{\mathrm{eff}} \approx 65.9$,
substantially higher than silicon ($Z_{\mathrm{eff}} = 14$), germanium (32),
or even CdZnTe ($Z_{\mathrm{eff}} \approx 49.1$)~\cite{Springer_CsPbBr3}.
...
\begin{equation}
  W = \sqrt{\frac{2\varepsilon\varepsilon_0 (V_{\mathrm{bi}} - V)}{e N_D}},
  \label{eq:depletion_width}
\end{equation}
where $\varepsilon$ is the static dielectric constant, $V_{\mathrm{bi}}$
...
\begin{equation}
  ...
  J_0 = A^{**}T^2\,e^{-q\phi_{\mathrm{B}}/kT},
...
For the 2D perovskite devices studied in Chapter~\ref{ch:perovskite_detectors},
...
Chapter~\ref{ch:perovskite_detectors} investigates the combination of 2D
...
MHyPbCl$_3$ results reported in Chapter~\ref{ch:perovskite_detectors} represent
...
in Chapter~\ref{ch:perovskite_detectors} extend this concept to the $n = 3$
...
and~\ref{ch:perovskite_detectors}.  The ABX$_3$ halide perovskite framework
...
the experimental chapters that follow.  The experimental investigation
begins in Chapter~\ref{ch:CsFAPbI} with photovoltaic-architecture
perovskite solar cells operating as current-mode monitors at zero bias,
establishing the radiation-response baseline before
Chapter~\ref{ch:perovskite_detectors} introduces the applied-bias,
direct-conversion detector geometries that enable pulse-by-pulse energy
spectroscopy.